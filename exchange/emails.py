...
from accounts.emails import BuildAbsoluteURIMixin


class ConfirmTicketOfferEmail(BuildAbsoluteURIMixin, LadderEmail):
    ...
    def get_context_data(self, **kwargs):
        kwargs = super(ConfirmTicketOfferEmail, self).get_context_data(**kwargs)
        ticket_match = self.args[0]
        kwargs.update({
            'confirm_url': self.reverse_absolute_uri('offer_confirm', kwargs={'pk': ticket_match.pk}),
...
send_offer_confirmation_email = ConfirmTicketOfferEmail.as_callable()
...
send_offer_accepted_email = MatchConfirmationEmail.as_callable(
    template_name='exchange/mail/offer_accepted.html',
)
send_request_fulfilled_email = MatchConfirmationEmail.as_callable(
    template_name='exchange/mail/request_fulfilled.html',
)
send_offer_rejected_email = MatchConfirmationEmail.as_callable(
