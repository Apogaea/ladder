...
def test_automatic_offer_creation_view(factories, user_client, models):
    ...
    response = user_client.get(reverse('offer-create'))
    ...
    response = user_client.post(reverse('offer-create'), {'is_automatch': True})
...
def test_manual_offer_creation_doesnt_auto_match(factories, user_client, models):
    ...
    response = user_client.post(reverse('offer-create'), {'is_automatch': False})
    assert response.status_code == status.HTTP_302_FOUND
